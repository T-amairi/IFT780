...
from models.CNNBlocks import ResBottBlock, DenseBlock
...
class yourUNet(CNNBaseModel):
    ...
    def __init__(self, num_classes=4, init_weights=True):
        ...
        self.conv_encoder5 = self._contraction_block(512, 1024)
        
        # Middle
        self.conv_mid_decoder = nn.Sequential(ResBottBlock(1024, 1024),
    ...
    def forward(self, x):
        ...
        # middle
        decode_middle =  self.conv_mid_decoder(encode_block5)

        # Decode
        decode_block4 = torch.cat((decode_middle, encode_block4), 1)
    ...
    def _contraction_block(in_channels, out_channels, kernel_size=3):
        ...
        block = nn.Sequential(
            ...
            ResBottBlock(out_channels, out_channels),
        )
        return block
    ...
    def _expansion_block(in_channels, out_channels, kernel_size=3):
        ...
        block = nn.Sequential(
            DenseBlock(in_channels),
            ResBottBlock(in_channels*2, in_channels, stride = 1),
            ResBottBlock(in_channels, out_channels, stride = 1),
