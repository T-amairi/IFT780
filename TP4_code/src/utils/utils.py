...
def centered_padding(image, pad_size, c_val=0):
    ...
    if image.ndim == 4:
        ...
        to_pad = np.array(to_pad).astype(int)
...
def centered_crop(image, crop_size):
    ...
    if image.ndim == 4:
        ...
        to_crop = np.array(to_crop, dtype=int)
...
def mean_dice(input: Tensor, target: Tensor, reduction: str = 'mean') -> Tensor:
    ...
    # If the number of labels is 0, this means that the target segmentation map
    # contains only background pixels.  In that case, return a random dice.
    # This situation can occur when using a very small batch size (e.g. 1 or 2)
    if labels.numel()==0:
      return torch.rand(1)
...
def extract_data_augment(test_set, test_set_no_norm, test_set_t1, test_set_t2, n_fig=5):
    for i in range(n_fig):
        fig, axs = plt.subplots(1,1)

        img, label = test_set[i]
        img_no_norm, label_no_norm = test_set_no_norm[i]
        img_transformed, label = test_set_t1[i]
        img_transformed1, label = test_set_t2[i]
        
        npimg = np.array(img)
        nimg_transformed = np.array(img_transformed)
        nimg_transformed1 = np.array(img_transformed1)

        npimg = np.array(img)
        plt.title("Image d'origine (avec Normalisation)")
        plt.imshow(np.transpose(npimg, (1, 2, 0)))
        plt.savefig(f'../figure/im{i}_origine_norm.png', bbox_inches='tight')


        npimg = np.array(img_no_norm)
        plt.title("Image d'origine (sans Normalisation)")
        plt.imshow(np.transpose(npimg, (1, 2, 0)))
        plt.savefig(f'../figure/im{i}_origine.png')

        fig, axs = plt.subplots(1,1)
        plt.title("Image transformé à l'aide de la transformation 1")
        plt.imshow(np.transpose(nimg_transformed, (1, 2, 0)))
        plt.savefig(f'../figure/im{i}_transmode_1.png', bbox_inches='tight')


        fig, axs = plt.subplots(1,1)      
        plt.title("Image transformé avec la transformation 2")
        plt.imshow(np.transpose(nimg_transformed1, (1, 2, 0)))
        plt.savefig(f'../figure/im{i}_transmode_2.png', bbox_inches='tight')