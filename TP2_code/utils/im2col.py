...
def get_im2col_indices(X_shape, Fheight, Fwidth, padding=(0, 0),
    ...
    I_intra = np.repeat(np.arange(Fheight, dtype=int), Fwidth)
    I_intra = np.tile(I_intra, channel)
    # I_stride: Ajoute l'effet de la stride verticale aux index
    # intra-colonne.
    I_stride = stride_height * np.repeat(np.arange(out_height, dtype=int), out_width)
    ...
    J_intra = np.tile(np.arange(Fwidth, dtype=int), Fheight * channel)
    # J_stride: Ajoute l'effet de la stride horizontale aux index
    J_stride = stride_width * np.tile(np.arange(out_width, dtype=int), out_height)
    ...
    ##################################################################
    K = np.repeat(np.arange(channel, dtype=int), Fheight * Fwidth).reshape(-1, 1)
